...
import {
  testOpenRouterKey, testOllamaEndpoint, testLocalOpenAIEndpoint, listOpenRouterModels,
} from '../utils/api';
...
import { LOCAL_MODEL_PACKS, isLocalPackConfigured } from '../config/localModelPacks';
import { Workflow, WorkflowStep, ExternalAsset } from '../types';
import 'react-native-get-random-values';
import { v4 as uuidv4 } from 'uuid';

const QUICK_MODELS = [
  { id: OPENROUTER_AUTO_FREE_MODEL, label: 'Free Round Robin Router (recommended)' },
...
const LOCAL_QUICK_MODELS = [
  { id: 'toxiclaw', label: 'ToxicLaw (Ollama local)' },
  { id: 'toxiclaw:latest', label: 'ToxicLaw latest tag' },
  { id: 'llama3.3', label: 'Llama 3.3 local' },
  { id: 'local-model', label: 'OpenAI-compatible local model' },
];

function displayModel(modelId: string): string {
  return modelId === OPENROUTER_AUTO_FREE_MODEL
    ? 'Free Round Robin Router'
    : modelId === 'toxiclaw'
      ? 'ToxicLaw (Ollama local)'
    : modelId;
}
...
function providerLabel(provider?: ApiProvider): string {
  return provider === 'openrouter' ? 'OpenRouter'
    ...
    : provider === 'localopenai' ? 'Local OpenAI'
    : 'Default provider';
}

function isFreeCatalogModel(model: { id: string; pricing?: any }): boolean {
...
export default function SettingsScreen() {
  ...
  const [providerSettings, setProviderSettingsState] = useState<ProviderSettings>({
    ...
    defaultModel: OPENROUTER_AUTO_FREE_MODEL,
  ...
  const [corePrefs, setCorePrefsState] = useState<Record<string, { preferredProvider?: ApiProvider; preferredModel?: string; preferredKeyId?: string }>>({});
  ...
  async function handleSaveNewKey() {
    ...
    setNewKeyError('');
    const res =
      newKeyProvider === 'openrouter' ? await testOpenRouterKey(secret)
      : newKeyProvider === 'ollama' ? await testOllamaEndpoint(secret)
      : await testLocalOpenAIEndpoint(secret);
    if (!res.ok) {
    ...
    const key: ApiKey = {
      ...
      label: newKeyLabel.trim() || (
        newKeyProvider === 'openrouter' ? 'OpenRouter Key'
        : newKeyProvider === 'ollama' ? 'Ollama'
        : 'Termux Local'
      ),
      secretKey: `apikey:${id}`,
    ...
    try {
      ...
      setNewKeyError(e?.message || 'Could not save API key. Please try again.');
      setTesting(false);
    }
  }
  ...
  async function openModelPicker() {
    ...
    if (orKey) {
      ...
      const free = models.filter(isFreeCatalogModel).map(m => m.id).sort();
      const paid = models.filter(m => !isFreeCatalogModel(m)).map(m => m.id).sort();
      setDynModels([...free, ...paid]);
    } else {
      setDynModels([]);
    }
  ...
  async function saveAsset() {
    ...
    try {
      ...
      setAssetError(e?.message || 'Could not save integration. Please try again.');
  ...
  const localOpenAIKeys = apiKeys.filter(k => k.provider === 'localopenai');
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Section title="API KEYS & PROVIDERS">
          <View style={styles.providerPickRow}>
            {(['openrouter', 'ollama', 'localopenai'] as ApiProvider[]).map(p => (
              <TouchableOpacity
                key={p}
              ...
              >
                <Text style={[styles.providerPickText, providerSettings.defaultProvider === p && { color: COLORS.text }]}>
                  {p === 'openrouter' ? 'OPENROUTER' : p === 'ollama' ? 'OLLAMA' : 'LOCAL OPENAI'}
                </Text>
          ...
          {providerSettings.defaultProvider === 'openrouter' && (
            <TouchableOpacity style={styles.modelPickRow} onPress={openModelPicker} testID="open-model-picker">
              <View>
                ...
                <Text style={styles.modelPickValue}>{displayModel(providerSettings.defaultModel)}</Text>
              </View>
          ...
          {providerSettings.defaultProvider === 'localopenai' && (
            <View style={styles.ollamaRow}>
              <Text style={styles.modelPickLabel}>LOCAL MODEL NAME</Text>
              <TextInput
                style={styles.input}
                value={providerSettings.localOpenAIModel}
                onChangeText={v => { setProviderSettingsState(s => ({ ...s, localOpenAIModel: v })); updateProviderSettings({ localOpenAIModel: v }); }}
                placeholder="local-model"
                placeholderTextColor={COLORS.muted}
              />
              <Text style={styles.hintText}>
                Use the model id exposed by your Termux llama-server `/v1/models` endpoint.
              </Text>
            </View>
          )}
          ...
          <Text style={[styles.keyListTitle, { marginTop: 10 }]}>LOCAL OPENAI ENDPOINTS ({localOpenAIKeys.length})</Text>
          {localOpenAIKeys.length === 0 ? <Text style={styles.emptyText}>No local endpoints yet</Text> : localOpenAIKeys.map(k => (
            <KeyRow key={k.id} k={k} preview={keyPreviews[k.id] || k.label} onToggle={() => toggleKeyActive(k)} onDelete={() => handleDeleteKey(k)} />
          ))}
        ...
        {/* Local mobile model packs */}
        <Section title="LOCAL MOBILE PACK">
          {LOCAL_MODEL_PACKS.map(pack => {
            const configured = isLocalPackConfigured(pack);
            return (
              <View key={pack.id} style={styles.localPackRow}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.localPackName}>{pack.name}</Text>
                  <Text style={styles.assetRef}>
                    {configured
                      ? `${pack.fileName} · ${(pack.sizeBytes / (1024 ** 3)).toFixed(2)} GB · ${pack.quantization || 'GGUF'}`
                      : 'Waiting for model URL, size, and checksum'}
                  </Text>
                  <Text style={styles.hintText}>{pack.description}</Text>
                </View>
                <Text style={[styles.localPackStatus, configured ? styles.localPackReady : styles.localPackPending]}>
          ...
          <Text style={styles.hintText}>
            First native build will download the pack after install, verify it, then run it on-device without API keys.
          </Text>
        </Section>
        ...
        <Section title="PER-AGENT MODEL PINNING">
          ...
          {DEFAULT_AGENTS.map(agent => {
            const pinnedModel = corePrefs[agent.id]?.preferredModel || agent.preferredModel;
            const pinnedProvider = corePrefs[agent.id]?.preferredProvider || agent.preferredProvider;
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.pinValueText}>
                    {providerLabel(pinnedProvider)} · {pinnedModel ? displayModel(pinnedModel) : 'Default model'} {pinnedKey ? `· key: ${pinnedKey}` : ''}
                  </Text>
                </View>
                <Text style={styles.modelPickChevron}>›</Text>
              </TouchableOpacity>
          ...
          <View style={styles.modal}>
            ...
            <ScrollView style={{ maxHeight: 460 }} keyboardShouldPersistTaps="handled">
              ...
              <TextInput style={styles.input}
                placeholder={
                  newKeyProvider === 'openrouter' ? 'Personal OpenRouter…'
                  : newKeyProvider === 'ollama' ? 'Home Ollama…'
                  : 'Termux llama-server…'
                }
                placeholderTextColor={COLORS.muted}
              ...
              <TextInput style={styles.input}
                placeholder={
                  newKeyProvider === 'openrouter' ? 'sk-or-v1-…'
                  : newKeyProvider === 'ollama' ? 'http://127.0.0.1:11434'
                  : 'http://127.0.0.1:8080'
                }
                placeholderTextColor={COLORS.muted}
                value={newKeySecret}
              ...
              <Text style={styles.hintText}>
                {newKeyProvider === 'openrouter'
                  ...
                  : newKeyProvider === 'ollama'
                    ? 'Point to any reachable Ollama server. Pull models first with `ollama pull llama3.3`.'
                    : 'Point to Termux llama.cpp server. Example: `llama-server -m model.gguf --host 127.0.0.1 --port 8080`.'}
              </Text>
            </ScrollView>
            ...
            <ScrollView style={{ maxHeight: 420 }}>
              ...
              {QUICK_MODELS.map(m => (
                <TouchableOpacity key={m.id}
                  ...
                  <Text style={styles.modelOptionId}>
                    {m.id === OPENROUTER_AUTO_FREE_MODEL
                      ? 'Rotates across live free OpenRouter models and active keys'
                      : m.id}
                  </Text>
                </TouchableOpacity>
              ))}
                ...
                <>
                  ...
                  {dynModels.slice(0, 60).map(m => (
                    <TouchableOpacity key={m}
                      ...
                      onPress={() => pickModel(m)}>
                      <Text style={styles.modelOptionId}>{displayModel(m)}</Text>
                    </TouchableOpacity>
                  ))}
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
          <View style={styles.modal}>
            {showAgentPin && (() => {
              ...
              const selectedProvider = mine.preferredProvider || ag.preferredProvider || providerSettings.defaultProvider;
              return (
                <>
                  ...
                  <ScrollView style={{ maxHeight: 480 }} keyboardShouldPersistTaps="handled">
                    <Text style={styles.fieldLabel}>PROVIDER</Text>
                    <TouchableOpacity
                      style={[styles.modelOption, !mine.preferredProvider && styles.modelOptionActive]}
                      onPress={async () => { await setCoreAgentPref(ag.id, { preferredProvider: undefined, preferredKeyId: undefined }); await loadAll(); }}>
                      <Text style={styles.modelOptionLabel}>Use built-in/default provider</Text>
                      <Text style={styles.modelOptionId}>
                        {ag.preferredProvider ? `${providerLabel(ag.preferredProvider)} is preconfigured for ${ag.name}` : 'Follow the default provider selected in Settings'}
                      </Text>
                    </TouchableOpacity>
                    {(['openrouter', 'ollama', 'localopenai'] as ApiProvider[]).map(p => (
                      <TouchableOpacity key={p}
                        style={[styles.modelOption, mine.preferredProvider === p && styles.modelOptionActive]}
                        onPress={async () => { await setCoreAgentPref(ag.id, { preferredProvider: p, preferredKeyId: undefined }); await loadAll(); }}>
                        <Text style={styles.modelOptionLabel}>{providerLabel(p)}</Text>
                        <Text style={styles.modelOptionId}>
                          {p === 'localopenai'
                            ? 'Use a Termux/llama.cpp OpenAI-compatible endpoint'
                            : p === 'ollama'
                              ? 'Use an Ollama-compatible local endpoint'
                              : 'Use OpenRouter API keys'}
                        </Text>
                      </TouchableOpacity>
                    ))}
                    ...
                    <TouchableOpacity
                      ...
                      <Text style={styles.modelOptionLabel}>Use built-in/default model</Text>
                      <Text style={styles.modelOptionId}>
                        {ag.preferredModel ? `${displayModel(ag.preferredModel)} is preconfigured for ${ag.name}` : 'Follow the global default model'}
                      </Text>
                    </TouchableOpacity>
                    {selectedProvider !== 'openrouter' && (
                      <View style={styles.ollamaRow}>
                        <Text style={styles.modelPickLabel}>CUSTOM LOCAL MODEL TAG</Text>
                        <TextInput
                          style={styles.input}
                          value={mine.preferredModel || ag.preferredModel || ''}
                          onChangeText={async v => {
                            await setCoreAgentPref(ag.id, { preferredModel: v.trim() || undefined });
                            await loadAll();
                          }}
                          placeholder={selectedProvider === 'ollama' ? 'toxiclaw' : 'local-model'}
                          placeholderTextColor={COLORS.muted}
                          autoCapitalize="none"
                          autoCorrect={false}
                        />
                    ...
                    {(selectedProvider === 'openrouter' ? QUICK_MODELS : LOCAL_QUICK_MODELS).map(m => (
                      <TouchableOpacity key={m.id}
                    ...
                    {apiKeys.filter(k => k.provider === selectedProvider).map(k => (
...
const styles = StyleSheet.create({
  ...
  localPackName: { color: COLORS.text, fontSize: 13, fontWeight: '700' },
  localPackStatus: { fontSize: 10, fontWeight: '800', letterSpacing: 1.2 },
  localPackReady: { color: COLORS.green },
  localPackPending: { color: COLORS.highlight },
  ...
  secondaryButton: { backgroundColor: COLORS.surfaceElevated, borderColor: COLORS.border },
