...
import { LEGACY_CORE_AGENT_IDS } from '../agents/config';
...
export interface CoreAgentPrefs {
  [agentId: string]: {
    preferredProvider?: 'openrouter' | 'ollama' | 'localopenai';
...
export async function getProjects(): Promise<Project[]> {
  ...
  const migrated = projects.map(p => {
    const hasOriginalSwarm = LEGACY_CORE_AGENT_IDS.every(id => p.agents.includes(id));
    if (hasOriginalSwarm && !p.agents.includes('toxiclaw')) {
      changed = true;
      return { ...p, agents: [...p.agents, 'toxiclaw'], updatedAt: Date.now() };
    }
    return p;
  });
  if (changed) {
    await AsyncStorage.setItem(KEYS.PROJECTS, JSON.stringify(migrated));
  }
  return migrated;
}
...
export async function setExternalAssetToken(secretKey: string, token: string): Promise<void> {
  ...
  const savedToken = await getSecret(secretKey);
  if (savedToken !== token) {
    throw new Error('Could not save integration token securely. Please try again.');
  }
}

// ───────────────────── API keys (secure-store for secrets, AsyncStorage for metadata) ─────────────────────
export type ApiProvider = 'openrouter' | 'ollama' | 'localopenai';
...
export async function getApiKeys(provider?: ApiProvider): Promise<ApiKey[]> {
  ...
  for (const k of all) {
    if (k.secretKey && await getSecret(k.secretKey)) {
      valid.push(k);
    } else {
      removedStale = true;
    }
  ...
  if (removedStale) {
    await AsyncStorage.setItem(KEYS.API_KEYS, JSON.stringify(valid));
  }
  return provider ? valid.filter(k => k.provider === provider) : valid;
}
...
export async function saveApiKey(key: ApiKey, plaintextSecret?: string): Promise<void> {
  ...
  if (plaintextSecret !== undefined && plaintextSecret !== '') {
    ...
    const savedSecret = await getSecret(key.secretKey);
    if (savedSecret !== plaintextSecret) {
      throw new Error('Could not save API key securely. Please try again.');
    }
  }
  const raw = await AsyncStorage.getItem(KEYS.API_KEYS);
  const all: ApiKey[] = raw ? JSON.parse(raw) : [];
...
export const OPENROUTER_AUTO_FREE_MODEL = 'openrouter/auto-free';
...
export async function getProviderSettings(): Promise<ProviderSettings> {
  ...
  // Auto-heal any users still on a deprecated or manually pinned free model.
  if (DEAD_MODEL_IDS.has(current.defaultModel) || current.defaultModel.endsWith(':free')) {
...
const EXPORTABLE_KEYS = [
  ...
  KEYS.PROVIDER_SETTINGS, KEYS.USAGE_LEDGER, KEYS.PINNED_MEMORIES,
  KEY_CORE_PREFS,
];
...
export async function importAll(jsonString: string): Promise<{ imported: number }> {
  ...
  for (const [k, v] of Object.entries(data)) {
    if (k === KEYS.API_KEYS || k.startsWith('cc_sec_apikey:')) continue;
